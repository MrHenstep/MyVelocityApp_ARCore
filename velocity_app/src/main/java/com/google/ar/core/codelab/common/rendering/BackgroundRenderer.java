...
public class BackgroundRenderer {
  ...
  public void createOnGlThread(Context context) throws IOException {
    ...
    ByteBuffer bbCoords = ByteBuffer.allocateDirect(quadCoordsData.length * FLOAT_SIZE);
    bbCoords.order(ByteOrder.nativeOrder());
    quadCoords = bbCoords.asFloatBuffer();
    quadCoords.put(quadCoordsData);
  ...
  public void setQuadCoords(float[] newCoords) {
    if (newCoords == null || newCoords.length != 8) {
      throw new IllegalArgumentException("quadCoords must be an array of 8 floats (4 vertices, 2 components each)");
    }
    quadCoordsData = newCoords;
    if (quadCoords != null) {
      quadCoords.clear();
      quadCoords.put(quadCoordsData);
      quadCoords.position(0);
  ...
  public float[] getQuadCoords() {
    return quadCoordsData.clone(); // Defensive copy
  }
  // Returns a reference to the internal FloatBuffer of screen-space quad coords
  public FloatBuffer getQuadCoordsBuffer() {
    quadCoords.position(0); // Ensure rewind
    return quadCoords;
  }

  // Returns a reference to the internal FloatBuffer of transformed texture coords
  public FloatBuffer getQuadTexCoordsBuffer() {
    quadTexCoords.position(0); // Ensure rewind
    return quadTexCoords;
  }

  public float[] getQuadTexCoords() {
    float[] texCoords = new float[8];
    quadTexCoords.position(0);           // Rewind buffer
    quadTexCoords.get(texCoords);        // Copy buffer to array
    return texCoords;
  }
