...
void main() {
   ...
   // convert to normalised height, i.e. [0, 1]
   float normalizedHeight = clamp((a_Position.z - kMinHeightMeters) / (kMaxHeightMeters - kMinHeightMeters), 0.0, 1.0);

   // convert normalised height to RGB vec3, plus transparency, alpha
   v_Color = vec4(PerceptColormap(normalizedHeight), 1.0);

   // write to special variable gl_Position
   gl_Position = u_ModelViewProjection * vec4(a_Position.xyz, 1.0);

   // and write point size to special variable gl_PointSize
